...
# Crear el mensaje JSON con K1, K2 y el nonce t_nb 
# (Convertimos los bytes a hexadecimal para poder usar JSON)
msg_TB = []
...
# Codificar el JSON a bytes para poder cifrarlo
datos_TB_bytes = json_TB.encode("utf-8")

# Cifrar el mensaje con AES-GCM usando la llave KBT 
aes_engine_TB = funciones_aes.iniciarAES_GCM(KBT)
cifrado_TB, mac_TB, nonce_TB = funciones_aes.cifrarAES_GCM(aes_engine_TB, datos_TB_bytes)

# Enviar los tres componentes por el socket
...
# Codificar el JSON a bytes para poder cifrarlo
datos_TA_bytes = json_TA.encode("utf-8")

# Cifrar el mensaje con AES-GCM usando la llave KAT 
aes_engine_TA = funciones_aes.iniciarAES_GCM(KAT)
cifrado_TA, mac_TA, nonce_TA = funciones_aes.cifrarAES_GCM(aes_engine_TA, datos_TA_bytes)
